...
The most distinctive finding is mechanistic. The reasoning-tuned Thinking model exhibits a \emph{negative} anchor-control contrast under explicit CoT (Figure~\ref{fig:anchor}), indicating that its reasoning chains distribute causal load across many steps rather than concentrating it in a few dominant anchors. Combined with Thinking's inability to suppress reasoning even under answer-only instructions, this suggests that hidden chain-of-thought in reasoning-tuned models is qualitatively different from hidden chain-of-thought in instruction-tuned models --- more deeply integrated, more prompt-invariant, and more diffuse. Future work should extend HCDS to additional model families, evaluate it under stochastic decoding, and probe whether the diffuse-causal-load pattern generalizes to other reasoning-trained models.
